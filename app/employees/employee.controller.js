			...
			success(function(data, status, headers, config){
				...
				for (var i = 0; i < $scope.employees.length; i++) {
					...
					if (i % $scope.employeesPerPage === 0) {
						$scope.pagedEmployees[Math.floor(i / $scope.employeesPerPage)] = [ $scope.employees[i] ];
					} else {
						$scope.pagedEmployees[Math.floor(i / $scope.employeesPerPage)].push($scope.employees[i]);
					}
				}
			}).
			error(function(data, status, headers, config){
